...
process.argv
  .slice(2).forEach(option=>{
    if(option.indexOf('dir=')===0){
      homePath = option.split('=').slice(1).join();
    } else {
      names.push( c(option) );
    }
  })
...
const sdkIndexSpecTmpl = `import {rootReducer, actions, selectors, /*loadFromDisk, saveToDisk*/ } from "./index";
...
names.map(async (n) => {
  ...
  if(!hasSdkFolder){
    ...
    fs.writeFileSync( dir("./index.spec.js"), sdkIndexSpecTmpl );
  }
  ...
  if(index.indexOf('moduleName') > -1){
    return console.error('already exsists in sdk/index.js, will not import '+moduleName+' again');
  }
